...
class BouncingBall(): 
    ...
    def bounce(self, screen, screen_width, screen_height):
...
while not done:
    ...
    mouse_position = pygame.mouse.get_pos()     #tuple value (x, y)
    ...
    text = "Here"                               #set string 
    size = font.size(text)                      #set size using Font.size method
    ren = font.render(text, 0, BLACK, WHITE)    #create new Surface to render text
    
    if mouse_pressed[0]:
        screen.blit(ren, (10, 10))                #blit text onto the corner
    
    if event.type == pygame.MOUSEBUTTONDOWN:
        pressed = True
        ball = BouncingBall(mouse_position[0], mouse_position[1], random.randint(5,10), 
            random.randint(5,10), random.randint(30,80), random.choice(possible_ball_colors))
        
    if pressed:
        ball.bounce(screen, SCREEN_WIDTH, SCREEN_HEIGHT)
